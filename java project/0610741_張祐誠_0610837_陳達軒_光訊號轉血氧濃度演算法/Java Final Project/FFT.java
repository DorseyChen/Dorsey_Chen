...
public class FFT {
	...
	public static Complex[] zeroPadding(Complex[]array)
	{
		Complex[]result;
		...
		result=new Complex[pow];
		for(int i=0;i<array.length;i++)
		{
			result[i]=new Complex(array[i].getReal(),array[i].getImage());  
		}
		for(int i=array.length;i<result.length;i++)
		{
			result[i]=new Complex(0,0);
		}
		show(result,"zeroPadding result");
    ...
    ***************************************************************************/
    ...
    public static void main(String[] args) { 
        ...
        for (int i = 0; i < n; i++) {
            //x[i] = new Complex(i, 0);
            x[i] = new Complex(new SecureRandom().nextDouble(), 0);
        }
        show(x, "x");
		...
		//zero padding for non-power-of-2 sequence
		Complex[] e=new Complex[Integer.parseInt(args[1])];
		for (int i = 0; i < e.length; i++) {
            
            e[i] = new Complex(new SecureRandom().nextDouble(), 0);
        }
		e=zeroPadding(e);
