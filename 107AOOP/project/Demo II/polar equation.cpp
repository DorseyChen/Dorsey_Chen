...
void GenerateXY_Polar(pts& x_y,const string& postfix);
...
long double EvaluatePostfix(string);
...
// Function to verify whether a character is alphabetic or not.
...
int main() 
{
	pts x_y;//to store points of the polar equation under cartesian coordinates (x,y).
	string expression;  
	cout<<"r=";
	...
	cout<<"GenerateXY_polar:\n";
	GenerateXY_Polar(x_y,postfix);
	return 0;
}
...
long double EvaluatePostfix(string postfix)
{
...
void GenerateXY_Polar(pts& x_y,const string& postfix)
{
	...
	pts::iterator ptsItr;
	string tmp=postfix;//the angular variables in tmp are to be replaced with real value.
	x_y.clear();
	for(long double i=0;i<=PI;i+=0.001)
	{
			for(int j=0;j<postfix.length();j++)
			{
				if(IsVariable(tmp.at(j)) )//replace the alphabetic variables with real angular values
				{
					tmp.replace(j,1,to_string(i));
					//cout<<tmp<<endl;
				}
			}
			theta_r.insert(make_pair(i,EvaluatePostfix(tmp)));
			tmp=postfix;
	}
	/*cout<<"theta r\n";
	for(ptsItr=theta_r.begin();ptsItr!=theta_r.end();ptsItr++)
	{
		cout<<ptsItr->first<<"  "<<ptsItr->second<<endl;
	}*/
	for(ptsItr=theta_r.begin();ptsItr!=theta_r.end();ptsItr++)
	{
